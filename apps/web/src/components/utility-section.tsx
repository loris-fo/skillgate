...
export function UtilitySection({ utility }: { utility: UtilityAnalysis }) {
  return (
    <section className="bg-surface-1 border border-border rounded-lg p-6 my-8">
      ...
      {/* What it does */}
      <div className="mb-5">
        <h3 className="text-text-primary font-medium mb-1">What it does</h3>
        <p className="text-text-secondary text-sm">{utility.what_it_does}</p>
      </div>

      {/* Use Cases */}
      {utility.use_cases.length > 0 && (
        <div className="mb-5">
          <h3 className="text-text-primary font-medium mb-1">Use Cases</h3>
      ...
      {utility.not_for.length > 0 && (
        <div className="mb-5">
          <h3 className="text-text-primary font-medium mb-1">Not For</h3>
          <ul className="list-disc list-inside text-text-secondary text-sm space-y-1">
      ...
      {utility.dependencies.length > 0 && (
        <div>
          <h3 className="text-text-primary font-medium mb-2">Dependencies</h3>
          <div className="flex flex-wrap gap-2">
            {utility.dependencies.map((dep) => (
              <span
                key={dep}
                className="bg-surface-3 text-text-secondary text-xs font-mono px-2 py-1 rounded"
              >
                {dep}
              </span>
            ))}
